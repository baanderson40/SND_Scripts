...
function AwaitAddonReady(name, timeoutSec)
    ...
    if not ok then
        ...
        Log("AwaitAddonReady timeout: %s (addon=%s Ready=%s Exists=%s Visible=%s IsVisible=%s IsOpen=%s)",
            tostring(name),
            tostring(a),
...
local function FindSummoningBellEntity()
    local names = GetSummoningBellNames()
    for i = 1, #names do
        local name = names[i]
        local bell = Entity and Entity.GetEntityByName and Entity.GetEntityByName(name) or nil
        if bell then
            Log("resolved bell '%s'", tostring(name))
            return bell
        end
    end

    Log("no summoning bell entity matched resolved names")
    return nil
end
...
function SuppressAR(arg)
    if IPC and IPC.AutoRetainer and IPC.AutoRetainer.SetSuppressed then
        IPC.AutoRetainer.SetSuppressed(arg)
    end
end
...
yield("/echo [Login Repricer] AutoBell polling loop started.")
...
if not IsBellSessionActive() then
    local bell = FindSummoningBellEntity()
    if not bell then
        Log("failed to resolve summoning bell")
        return
    end

    if not InteractWithEntity(bell, 5.0) then
...
if IsBellSessionActive() or WaitUntil(IsBellSessionActive, 5, TIME.POLL, 0.0) then
    ...
    Log("failed to open bell session")
    return
end

if GetCharacterCondition(CharacterCondition.occupiedSummoningBell, true) then
    WaitUntil(function() return not GetCharacterCondition(CharacterCondition.occupiedSummoningBell) end, 999999, TIME.POLL, 1)
end
